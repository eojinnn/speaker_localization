...
class LibriSpeechLocations():
    ...
    '''
    def __init__(self, source_locs):
        self.source_file = "F:/speaker localization/44100_final.wav"
        self.source_locs = source_locs

    def __len__(self):
        return len(self.source_locs)
    
    def __getitem__(self, n: int) -> Tuple[Tensor, int, str, int, int, int, int, float, int]:
        source_loc = self.source_locs[n]
        seed = n
        waveform, sample_rate = librosa.load(self.source_file)
        waveform = torch.tensor(waveform)
        return (waveform, sample_rate), source_loc, seed
...
class DelaySimulator(object):
    ...
    def __call__(self, batch):
        ...
        with torch.no_grad():
            for (waveform, sample_rate), source_loc, seed in batch:
