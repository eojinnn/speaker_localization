...
DATA_LEN = 2620
NUM_TEST_WINS = 5
MIN_SIG_LEN = 19  # only use snippets longer than 2 seconds
...
# create datasets
train_set = LibriSpeechLocations(source_locs_train)
...
#             > fs * MIN_SIG_LEN else None for i, ((waveform, sample_rate,), pos, seed)
#             in enumerate(train_set)]

valid_idx = [i for i in range(2620)]
...
val_set = LibriSpeechLocations(source_locs_val)
test_set = LibriSpeechLocations(source_locs_test)

# 전체 데이터의 인덱스를 추출
indices = list(range(len(train_set)))

# train(70%), val(15%), test(15%) 비율로 나누기
indices_train, indices_temp = train_test_split(indices, test_size=0.1, random_state=42)
indices_val, indices_test= train_test_split(indices_temp, test_size=0.5, random_state=42)
...
(waveform, sample_rate), pos, seed = train_set[0]
...
model.eval()
summary(model, [(1, 1, sig_len), (1, 1, sig_len)]) # model input x1 = (1,1,sig_len), x2 = (1,1,sig_len)
...
for e in range(epochs):
    ...
    with tqdm(total=len(train_set)) as pbar:
        for batch_idx, (x1, x2, delays) in enumerate(train_loader):
            ...
            gt = delays - max_tau ##ground truth
    ...
    with tqdm(total=len(val_set)) as pbar:
        for batch_idx, (x1, x2, delays) in enumerate(val_loader):
            with torch.no_grad():
                ...
                cc = gcc(x1.squeeze(), x2.squeeze())
                shift_gcc = torch.argmax(cc, dim=-1) - max_tau_gcc 
                ##torch.argmax(cc)는 [-max_tau,...,max_tau]에서 마이너스부터 시작해서 계산한 인덱스
                ##-max_tau_gcc를 해줘야함.
